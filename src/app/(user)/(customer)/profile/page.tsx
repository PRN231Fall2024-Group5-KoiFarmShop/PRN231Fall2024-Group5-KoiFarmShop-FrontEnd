...
export default function ProfilePage() {
  ...
  const handleWithdraw = async () => {
    ...
    setLoading(true);
    const response = await withdrawnRequestAPI.createWithdrawnRequest(
      bankNote,
      amount.toString(),
    );
    setLoading(false);
  ...
  const isAmountValid = (amount: number) => {
    if (isNaN(amount) || amount < 20000 || amount > 10000000) {
      toast({
        ...
        description:
          "Please enter an amount between 20,000 VND and 10,000,000 VND",
        variant: "destructive",
      });
      return false;
    }
